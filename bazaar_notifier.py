...
from minescript import EventQueue, EventType, chat, echo
...
def load_conversions():
    ...
    # For this script, I'll include a subset of the provided JSON for demonstration,
    # or try to load it if it exists.
    try:
        # Assuming the script is run from a place where it can access the file, 
        # or we paste the content. Since I can't easily rely on file paths in the user's 
        # minecraft folder, I will embed the critical logic or a small map, 
        # but ideally we would load the full JSON.
        # For this implementation, I will assume the user might have the file or I'll use a simple heuristic.
        # But to be robust, let's include the map I read earlier.
        
        # (Truncated for brevity in this thought process, but I will include the full map in the actual file write)
        pass 
    except Exception as e:
        echo(f"Error loading conversions: {e}")
...
def update_orders_from_api():
    global my_orders
    try:
        response = requests.get(API_URL)
        if response.status_code != 200:
            echo(f"Bazaar API Error: {response.status_code}")
            return
        ...
        for order in my_orders:
            ...
            top_price = top_market_order['pricePerUnit']
            ...
            if new_status != order.status:
                if new_status == "OUTDATED":
                    chat(f"&c[BN] {order.order_type.value} for {order.product_name} is OUTDATED!")
                elif new_status == "MATCHED":
                    chat(f"&e[BN] {order.order_type.value} for {order.product_name} is MATCHED.")
                elif new_status == "BEST" and order.status == "OUTDATED":
                    chat(f"&a[BN] {order.order_type.value} for {order.product_name} is BEST again.")
                
                order.status = new_status

    except Exception as e:
        echo(f"API Update Error: {e}")
...
def process_chat(message):
    ...
    if match:
        ...
        for i, order in enumerate(my_orders):
            if order.product_id == p_id and order.order_type == o_type:
                # Ideally check amount, but amount changes as it fills. 
                # For simplicity, remove the first matching one.
                del my_orders[i]
                echo(f"&a[BN] Order filled and removed: {p_name}")
                break
        return

    # 3. Cancelled
    # "Bazaar! You cancelled your Sell Offer for 64x Enchanted Iron!"
    cancelled_pattern = r"Bazaar! You cancelled your (Buy Order|Sell Offer) for ([\d,]+)x (.+)!"
    ...
    if match:
        ...
        for i, order in enumerate(my_orders):
            if order.product_id == p_id and order.order_type == o_type:
                del my_orders[i]
                echo(f"&c[BN] Order cancelled and removed: {p_name}")
                break
        return

# --- Main ---

# Embed the conversions map here for the script to be standalone
BAZAAR_CONVERSIONS_RAW = {
  ...
  "ARACHNE_KEEPER_FRAGMENT": "Arachne's Keeper Fragment",
  ...
  "NECROMANCER_BROOCH": "Necromancer Brooch",
...
if __name__ == "__main__":
    echo("&e[BN] Starting Bazaar Notifier...")
    
    # Initialize conversions
    bazaar_conversions = BAZAAR_CONVERSIONS_RAW
    bazaar_conversions_inverse = {v: k for k, v in bazaar_conversions.items()}
    ...
    with EventQueue() as event_queue:
        ...
        echo("&a[BN] Listening for Bazaar chat messages...")
