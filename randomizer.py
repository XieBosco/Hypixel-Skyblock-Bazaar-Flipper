...
class HumanDelay:
    """
    Simulates human-like delays for bot actions to avoid detection.
    """
    ...
    def get_delay(self, min_seconds=2.5, max_seconds=6, skew=0.5, momentum=0.4) -> float:
        """
        Calculates a randomized delay duration based on a log-normal distribution and momentum.

        Args:
            min_seconds (float): Absolute minimum wait time (physical limit).
            max_seconds (float): Target maximum wait time.
            skew (float): Controls the 'tail' of the distribution (0.1 = tight, 0.5 = long tail of slow clicks).
            momentum (float): 0.0 to 0.9. How much the previous delay affects the new one.
